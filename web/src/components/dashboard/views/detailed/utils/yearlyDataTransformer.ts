...
export function transformYearlyDataToGrid(data: YearlyData, collapsedSections: CollapsibleState = {}, customProjectedValues: Record<string, number> = {}, _physicianData?: { physicians: Physician[], benefitGrowthPct: number, locumCosts: number, prcsDirectorPhysicianId?: string | null, prcsMedicalDirectorHours?: number, medicalDirectorHours?: number, consultingServicesAgreement?: number }, gridSnapshot?: Record<string, number> | null): { rows: Row[], columns: any[], allRows: Row[] } {
  ...
  // Helper to normalize account name for customProjectedValues lookups (same as YearlyDataGrid)
  const normalizeForCustomValues = (accountName: string): string => {
    return (accountName || '')
  ...
  const getMainProjectedByPattern = (pattern: RegExp): number => {
    ...
    for (const r of flattenedRows) {
      ...
      if (pattern.test(normalized)) {
        // Check both normalized and original names
        const normalizedNm = normalizeForCustomValues(nm)
        const customValueOverride = customProjectedValues[normalizedNm] ?? customProjectedValues[nm]
        if (customValueOverride !== undefined) {
          return Number(customValueOverride) || 0
        }
  ...
  const getProjectedNumericForRow = (r: any): number => {
    ...
    // Check both normalized and original names
    const normalizedName = normalizeForCustomValues(name)
  ...
  const allDataRows: Row[] = visibleRows.map((row, index) => {
    ...
    const cells = row.colData.map((cellData: any, cellIndex: number) => {
      ...
      // Normalize accountName for lookup since customProjectedValues keys don't have info icons
      const normalizedAccountName = normalizeForCustomValues(accountName)
      const hasCustomValue = isProjectedColumn && (customProjectedValues[normalizedAccountName] !== undefined || customProjectedValues[accountName] !== undefined)
...
function addSummaryRows(data: YearlyData, projectionRatio: number): any[] {
  ...
  summaryRows.push({
    ...
    computed: true,  // Mark as computed so grid transform dynamically sums the Data rows above
    type: 'Section',
    group: 'SummaryIncome'
  })
